...
### Gemini Omni Flash (Google) - cloud / third-party only, no official local node yet
...
- **Access reality (confirmed via live ComfyUI 0.25.1 `/object_info` + the Comfy-Org/workflow_templates `index.json`, checked 2026-06-30):** there is NO official Comfy Google partner node and NO local template for Gemini Omni Flash. It runs on **Comfy Cloud** (cloud.comfy.org hosted workflows) and through a **third-party** community pack that routes to the **muapi.ai** Gemini Omni API (`github.com/Anil-matcha/gemini-omni-comfyui`: T2V / I2V up to 5 reference images / video-edit). It is not installable as an official local node today. Local-first call: use Comfy Cloud or that third-party pack (needs a muapi key + credits), or pick a video model that HAS an official local node - Veo 3, Kling, Seedance, Sora.
- **Marketing to ignore:** the launch line "replaces Veo entirely" is overstated. Google's own model card lists Veo and Gemini Omni Flash as separate models, and ComfyUI's Veo 2 / Veo 3 partner nodes are live and not deprecated (confirmed). Treat Omni Flash as an addition, not a Veo replacement.
- **Prompt style (inferred from the Gemini / Veo family plus the conversational-edit design; verify against the muapi / Google Flow docs when you actually run it):** natural language, subject -> action -> setting -> camera -> audio; for an edit, give a plain conversational instruction ("make it night, add rain on the window, keep the actor"). Audio renders with the visual, so name dialogue / SFX / ambience in the prompt.
- **Source:** deepmind.google/models/model-cards/gemini-omni-flash (2026-05-19) ; muapi.ai + github.com/Anil-matcha/gemini-omni-comfyui (third-party ComfyUI route) ; cloud.comfy.org (Comfy Cloud). Not in Comfy-Org/workflow_templates as of 2026-06-30.
